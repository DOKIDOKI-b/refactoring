...
const previousDateFromNow = (days: number) =>
  new Date(Date.now() - 1000 * 60 * 60 * 24 * days);

const recentDateCutoff = () => previousDateFromNow(3);

const renderPhoto = (outStream: OutputStream, aPhoto: Photo) => {
  outStream.write(`<img src="${aPhoto.url}" />`);
};

const emitPhotoData = (outStream: OutputStream, photo: Photo) => {
  outStream.write(`<p>제목: ${photo.title}</p>`);
  outStream.write(`<p>날짜: ${photo.date.toDateString()}</p>`);
  outStream.write(`<p>위치: ${photo.location}</p>`);
};

const listRecentPhotos = (outStream: OutputStream, photos: Array<Photo>) => {
  photos
...
const renderPerson = (outStream: OutputStream, person: Person) => {
